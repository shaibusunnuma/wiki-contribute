...
export default ({ property, setModalType, setIsModalVisible, setOldValue, setValue, startCamera }) => {
    const { setSelectedPropertyPID } = React.useContext(WikiContext);
    let Value: JSX.Element;
    if (property.wdLabel.value.includes("image")) {
        Value = <ImageValue value={property.ps_Label.value} />;
    } else if (property.wdLabel.value.includes("coordinate location")) {
        let value = property.ps_Label.value;
        value = value.substring(0, value.length - 1);
        const coord = value.split("(")[1].split(" ");
        Value = <MapValue value={coord} />;
    } else if (property.wdLabel.value.includes("website")) {
